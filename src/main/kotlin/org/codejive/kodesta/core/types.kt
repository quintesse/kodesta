package org.codejive.kodesta.core
...
fun Enums.enumById(enumId: String): List<Enumeration> {
    return this[enumId] ?: listOf();
}

fun Enums.enumItem(enumId: String, itemId: String): Enumeration? {
    return enumById(enumId).find { e -> e.id == itemId }
}

fun Enums.enumNN(enumId: String): List<Enumeration> {
    return getValue(enumId)
}

fun Enums.enumItemNN(enumId: String, itemId: String): Enumeration {
...
fun validRuntime(runtime: Runtime, enums: Enums): Runtime {
    val rt = enums.enumById("runtime.name").find { rt -> rt.id == runtime.name }
    if (rt == null) {
        throw IllegalArgumentException("Unknown runtime '${runtime.name}'")
    }
    val versions = enums.enumById("runtime.version.${runtime.name}")
