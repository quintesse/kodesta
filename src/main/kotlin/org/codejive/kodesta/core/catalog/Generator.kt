...
import org.codejive.kodesta.core.template.transform
import org.codejive.kodesta.core.template.transformFiles
...
abstract class BaseGenerator(protected val info: GeneratorInfo, protected val ctx: GeneratorContext) : Generator {
    ...
    protected fun generator(genName: String): Generator {
        val genInfo = ctx.registry.byName(genName)
    ...
    protected fun copyVersioned(runtime: Runtime?, to: Path? = null) {
        if (runtime != null) {
            val source = Paths.get("files-${validRuntime(runtime, ctx.registry.enums).version}")
            copy(source)
        }
    }
    ...
    protected fun mergeVersionedPoms(runtime: Runtime?, targetPom: Path = PATH_POM) {
        if (runtime != null) {
            val sourcePom = Paths.get("merge-${validRuntime(runtime, ctx.registry.enums).version}/pom.xml")
            mergePoms(sourcePom)
        }
    }
    ...
    protected fun mergeVersionedPackageJson(runtime: Runtime?, target: Path = PATH_PACKAGE) {
        if (runtime != null) {
            val sourceJson = Paths.get("merge-${validRuntime(runtime, ctx.registry.enums).version}/package.json")
    ...
    protected fun addVersionedResources(resources: Resources, transformer: Transformer, runtime: Runtime?) {
        if (runtime != null) {
            val source = Paths.get("resources-${validRuntime(runtime, ctx.registry.enums).version}")
...
private val fileAttrs: Map<Path, Attrs> by lazy {
    val fileAttrFile = Paths.get("META-INF/fileattr.yaml")
    if (existsFromPath(fileAttrFile)) {
        val f = yamlIo.arrayFromStream(streamFromPath(fileAttrFile))
        f.map {
            resolveClassPath(
                Paths.get(
                    "META-INF",
                    it.get("file") as String
                )
            ) to Attrs.build(it.get("attr") as Properties)
        }.toMap()
    } else {
        mapOf()
    }
