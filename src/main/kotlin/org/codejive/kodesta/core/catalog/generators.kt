...
import org.codejive.kodesta.catalog.AppImagesProps
...
import org.codejive.kodesta.core.template.transformers.*
...
class SimpleConfigGenerator(info: GeneratorInfo, ctx: GeneratorContext) : BaseGenerator(info, ctx) {
    ...
    override fun apply(resources: Resources, props: Properties, extra: Properties): Resources {
        ...
        if (config.actionsOnce.isNotEmpty()) {
            ...
            config.base?.let {
                generator(it).apply(resources, blprops, extra)
            }
            if (resources.service(blprops.serviceName) == null || !filesCopied() || !filesCopiedVersioned(blprops.runtime)) {
                if (config.image != null) {
                    ...
                    generator("app-images").apply(resources, biprops, extra)
                }
                if (info.infoDef.module.startsWith("runtime-")) {
                    // For runtime generators apply the base runtime support generator
                    generator("runtime-base-support").apply(resources, blprops, extra)
                }
    ...
    private fun defaultFileActions(
        ...
        if (existsFromPath(sourceDir.resolve(FILE_FILES_POM)) && props.containsKey("maven")) {
            generator("maven-setup").apply(resources, props, extra)
        }
    ...
    private fun updateExtra(extra: Properties, withProps: Properties) {
        ...
        extra.pathGet<Any?>("shared.runtimeInfo.enumInfo")?.let {
            if (it is String && it.isNotBlank()) {
                extra.pathPut("shared.runtimeInfo.enumInfo", ctx.registry.enums.enumItemNN("runtime.name", it))
            }
        }
    }
}
